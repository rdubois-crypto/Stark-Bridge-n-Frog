...
function FCL_tweak_initiator(uint256 hash, uint256 privA, uint256 random) view
...
function FCL_tweak_responder(uint256 hash, uint256 privB, uint256 random)  
pure
returns(uint256 SB) 
{
...
function FCL_unlock_initiator(uint256 SpA, uint256 SB) pure
...
function FCL_unlock_responder(uint256 SpA, uint256 SB) pure
returns(uint256 S_AB ) 
{
  /* randomness is derived to produce tweak t and nonce rA */  
  S_AB=addmod(SpA, SB, p);
